...
in
{
  ...
  config = lib.mkIf cfg.packages.zellij.enable {
    ...
    programs = {
      ...
      bash.initExtra = ''
        if [ -z "$ZELLIJ" ] && [ -z "$SSH_CONNECTION" ] && [ "$TERMINAL_EMULATOR" != "JetBrains-JediTerm" ] && [ "$TERM_PROGRAM" != "vscode" ] && [ "$ZED_TERM" != "true" ]; then
      ...
      fish.interactiveShellInit = ''
        if not set -q ZELLIJ && not set -q SSH_CONNECTION && not test "$TERMINAL_EMULATOR" = "JetBrains-JediTerm" && not test "$TERM_PROGRAM" = "vscode" && not test "$ZED_TERM" = "true"
