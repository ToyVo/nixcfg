...
in
{
  ...
  config = lib.mkIf cfg.packages.zellij.enable {
    ...
    programs = {
      ...
      bash.initExtra = ''
        if [ -z "$ZELLIJ" ] && [ -z "$SSH_CONNECTION" ] && [ "$TERMINAL_EMULATOR" != "JetBrains-JediTerm" ] && [ "$TERM_PROGRAM" != "vscode" ]; then
      ...
      fish.interactiveShellInit = ''
        if not set -q ZELLIJ && not set -q SSH_CONNECTION && not test "$TERMINAL_EMULATOR" = "JetBrains-JediTerm" && not test "$TERM_PROGRAM" = "vscode"
      ...
      nushell.configFile.text = ''
        if "ZELLIJ" not-in $env and "SSH_CONNECTION" not-in $env and $env.TERMINAL_EMULATOR? != "JetBrains-JediTerm" and $env.TERM_PROGRAM? != "vscode" {
